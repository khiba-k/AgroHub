...
const CartItems = ({
  ...
  totalItems: number; // represents sum of selectedQuantity OR count of unique lines, depending on Cart.tsx
  searchQuery: string;
                ...
                <div className="flex items-center justify-between flex-col md:flex-row">
                  ...
                  <div className="flex-1">
                    <h3 className="text-lg font-medium">
                      {item.produceType ? `${item.produceType} ${item.produceName}` : item.produceName}
                    </h3>
                    <p className="text-lg font-semibold mt-2">
                      LSL {item.totalPrice.toFixed(2)} ({item.selectedQuantity} {item.unitType})
                    </p>
                    ...
                    <div className="mt-2 text-sm text-gray-600">
                      ...
                      <ul className="list-disc list-inside">
                        {item.orderBreakdown.map((ob) => (
                          <li key={ob.farmerId}>
                            {ob.farmerName}: {ob.quantity} {item.unitType} @ LSL {(ob.price / ob.quantity).toFixed(2)} each
                          </li>
                        ))}
                      </ul>
                    </div>
                  ...
                  <div className="flex items-center space-x-4 mt-4 md:mt-0">
                    <div className="flex items-center space-x-2">
                      <label htmlFor={`quantity-${item.produceId}`} className="text-sm font-medium">
                        Quantity:
                      </label>
                      <Input
